...
const CustomTable: React.FC<CustomTableProps> = (props) => {
	...
	const filteredItems = React.useMemo(() => {
		...
		const hasTaxi = filtered.some((item) => typeof item.taxi !== 'undefined');
		const hasClient = filtered.some((item) => typeof item.cliente !== 'undefined');
		const hasRecogidaDate = filtered.some((item) => typeof item.recogida_date !== 'undefined');
		const hasReservaDate = filtered.some((item) => typeof item.reserva_date !== 'undefined');
		const hasCompletado = filtered.some((item) => typeof item.completado !== 'undefined');
		...
		filtered = filtered.map((item) => {
			...
			if (hasMarcaId && typeof item.marca_id !== 'undefined') {
				const {id, nombre} = marcaData.find(({id}) => id === item.marca_id)
				newItem = {...newItem, marca_id: `${id} ${nombre}`};
			}

			if (hasTaxi && typeof item.taxi !== 'undefined') {
				const taxiInfo = allApiData?.taxi.find(({id}) => id === item.taxi);
				newItem = {...newItem, taxi: taxiInfo ? `${taxiInfo.id} ${taxiInfo.chofer}` : item.taxi};
			}
			...
			if (hasCompletado && typeof item.completado !== 'undefined') {
				newItem = {...newItem, completado: item.completado ? 'Sí' : 'No'};
			}
